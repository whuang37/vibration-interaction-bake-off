...
class StreamAnalyzer:
    ...
    def __init__(self,
        ...
        try:
            from stream_reader import StreamReaderPyAudio
            self.stream_reader = StreamReaderPyAudio(
                device  = device,
                rate    = rate,
                updates_per_second  = updates_per_second,
                verbose = verbose)
        except:
            from stream_reader import StreamReaderSoundDevice
            self.stream_reader = StreamReaderSoundDevice(
                device  = device,
                rate    = rate,
                verbose = verbose)
        ...
        if self.smoothing_length_ms > 0:
            ...
            self.feature_buffer = NPDataBuffer(len(self.smoothing_kernel), len(self.fft), dtype = np.float32, data_dimensions = 2)
        ...
        self.rolling_bin_values = NPDataBuffer(self.rolling_stats_window_n, self.n_frequency_bins, start_value = 25000)
