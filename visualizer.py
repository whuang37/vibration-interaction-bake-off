...
from models import train_model, load_model, save_model, get_ear, get_audio_features, preprocess_data, predict
...
def frame_callback():
    ...
    if dpg.get_value("record_toggle") and record_val:
        ...
        dpg.set_value(f'cls_{record_val}', f"{record_val}: {len(collected_data[record_val])}")
...
with dpg.window(tag='fft_vis', label='FFT Spectrum', no_move=True, no_resize=True, no_collapse=True):
...
with dpg.window(tag='ctrl_win', label='Controls', no_move=True, no_resize=True, no_collapse=True):
    # Mode switch combo
    dpg.add_text('Mode:')
    dpg.add_combo(items=['Train', 'Infer'], default_value='Train', tag='mode',
    ...
    with dpg.group(tag='infer_group', show=False):
        with dpg.group(horizontal=True):
            dpg.add_button(label='Save Model', callback=save_model_callback, tag='btn_save')
            dpg.add_button(label='Load Model', callback=load_model_callback, tag='btn_load')
        dpg.add_text('Classes:', bullet=True)
        dpg.add_listbox(items=infer_classes, tag='infer_classes_list', num_items=5, default_value="sss")
        with dpg.group(horizontal=True):
            dpg.add_text('Prediction: ')
            dpg.add_text('---', tag='pred_text')

# File dialogs
with dpg.file_dialog(tag='load_model_dialog', show=False, callback=load_model_dialog_callback,
                     width=700, height=400, modal=True, default_path=".", label="Choose Model to Load"):
    dpg.add_file_extension('.model')

with dpg.file_dialog(tag='save_model_dialog', show=False, callback=save_model_dialog_callback, label="Choose Save Location",
                     width=700, height=400, modal=True, default_filename="model", file_count=1, default_path="."):
    dpg.add_file_extension('.model')
